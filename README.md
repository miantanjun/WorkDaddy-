...
| 功能新增 | **云端残留检测与清理**：桌面删除只删本机，而手机端/其它电脑读的是**云端那份**（删除不会通知云端），于是「桌面删干净了、手机端还在」。会话页新增可折叠的「云端残留」卡片：列出「本机已删、云端仍在」的会话，按账号分组清理；跨账号的残留必须切到该账号才能清（云侧按当前登录账号鉴权），按钮走两段式确认、清完自动切回。**删除会话时也会顺带清一次云端**，从源头不再产生残留 |
| 功能改进 | 删除确认框补一句「本机删除不会同步到其它设备，手机端／其它电脑上仍可能看到这些会话」 |
