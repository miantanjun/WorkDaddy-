...
**本版主题**：**吸纳上游 1.2.7** —— 上游基线从 `1.2.6` 提到 **`1.2.7`**；同时补上一项自研能力（复制整个会话）与 Linux 支持。

**真增量**（tree blob-SHA 集合差，排除 `scripts/runtime/`）：新增 **2** / 删除 **0** / 改动 **13**，其中 10 个是上游自带 `test/` ⇒ **产品面只有 5 个改动文件、0 个新模块**。符号级四分类实测 **删除 0 个函数、纯删 hunk 0、大幅压缩 hunk 0** ⇒ **纯增量版本**（不存在「上游删旧实现导致计划推翻」的风险）。

**质量门**：回归 **52 套件 / 3606 断言全绿**（v1.8.0 为 51 / 3511，本版 +1 套件 / +95 断言）；护栏 `tools/updater-guard.js` **18 PASS**；打包白名单传递闭包 **61 可达模块 / 0 缺失**；daemon `1.9.0`。

**基线核验**（唯一有效证据，不看「有没有新版」那个布尔）：`GET /api/update-check` → `upstreamVersion 1.2.7 === upstreamLatest 1.2.7`、`upstreamError null`、`checkedVia api`。

### 新增功能

- **复制整个会话为 Markdown**（`c47e942`）：面板「单会话成本卡」头部新增一键复制，把当前会话还原成「用户：…／`---`／助手：…」的完整 Markdown 直接进剪贴板。取消息走**官方消息 store**（会话列表会虚拟化，DOM 里只有可见部分，读 DOM 会漏内容），并剥掉内部的完成标记（`[wbs-reply-done]`）。
  ⚠️ 这是**自研实现**：上游把同一能力挂在一个本仓**从来就没有**的模块上（`wbs-session-usage-*`，30 个函数），本版**只取其中一个自包含纯函数**（`conversationMessagesToMarkdown`），入口挂本地既有载体、复用既有的会话控制器查找与剪贴板实现。
- **Linux 支持**（`c47e942`）：① 防休眠走 `systemd-inhibit`（detached 起、结束按**整进程组**回收，不留后台进程）；② 立即休眠走 `systemctl suspend`；③ 打开授权链接走系统默认浏览器 `xdg-open`（缺 `xdg-utils` 时返回可读错误而**不让守护进程崩**，且**不记录含登录 state 的 URL**）。三段全部由 `IS_LINUX` 守卫 ⇒ **Windows / macOS 零行为变化**。
- **毛玻璃主题适配新版 WorkBuddy**（`c47e942`）：新版 `teams` 布局给滚动容器与内部网格加上了不透明底色，把毛玻璃背景挡住（看起来像主题失效）。新增 **patch-102 / 103 / 104**（滚动容器与 `_gridView_` 透明化 / 发送按钮 tooltip 包装层 / 模板切换按钮与代码块复制提示），并移除其自身的 `backdrop-filter`。主题补丁总数 96 → **99**。

### 缺陷修复

- **积分统计里模型名显示成一坨看不懂的东西**（`c47e942`）：账单接口的模型元数据变过多种形状，旧的 `safeText(row.model)` 遇到**对象**只会得到 `[object Object]` ⇒ 用量表与积分趋势里的模型列全是同一坨字符。现按 7 个候选键**递归**取值（字符串 / 对象 / 嵌套对象 / 数字四种形状都认得）。
- **面板「关闭」按钮点不到 + 窗口矮时面板顶出屏幕**（`c47e942`）：关闭按钮被同容器的层叠上下文盖住；面板高度写死 `650px`，窄高窗口下超出可视区。现给关闭按钮抬层级，高度改成 `calc(100vh - 110px)`。

### 优化调整

- **积分日汇总缓存口径版本化**（`c47e942`）：`CREDIT_HISTORY_CACHE_VERSION` `1 → 2`。**必须与上面的模型名归一化同批** —— 版本号抬升的唯一目的就是丢弃按**旧口径**归一化的 `models{}` 键；拆开做会「一个白丢有效缓存、一个静默错配模型名」。
  ⚠️ 副作用：旧缓存（本机 270 行）被忽略，需重新向官方拉取一次（已在本机执行：覆盖 `2026-06-29 → 09-26`，90 天 × 3 账号 = 270 行，0 失败）。**权威逐请求表 `credit-usage.db` 不受影响**，用量看板不会倒退。
- **`toast()` 支持第 4 个参数 `level`**（`c47e942`）：`level || (isErr ? 'error' : 'info')` ⇒ **缺省回落 `isErr`，旧的三参调用语义逐字不变**（新增能力为「成功级提示」，用于复制成功这类不该按错误样式提示的场景）。

### 判定不做

- **上游 `shouldStartAutoCopyJob` 重构**（上游 1.2.7「切号时只同步用户明确选择的会话」）：**语义本地已覆盖，且更彻底** —— 本地重构出的 `autoCopyAfterAccountSwitch()`（`daemon.js:8548`）调的 `startAutoCopyJob` **签名里根本没有 `openSessionId` 参数**，上游要修的那条「打开着的会话被强制纳入复制计划」的路径本地不存在。
  ⚠️ 已在测试里登记守卫：谁哪天给 `startAutoCopyJob` 加回 `openSessionId`，必须同时补 `isAutoCopySessionSelected` 门控。
- **上游「会话用量」模块（30 个函数）**：本地有**面板版**「单会话成本卡」+ 统一用量看板，与上游「输入框旁徽标 + hover 明细」是**两种使用场景**（一个胜在能对比分析，一个胜在随手可见），整合风险远大于收益。
- **上游「会话副本通知卡」（≈16 个函数）**：本地自研版功能更全（多了**暂停/继续**、**冲突详情**、**立即同步**入口），采纳等于用更弱的实现替换更强的。

### 工程与文档

- 新增守卫套件 `.wd-analysis/test-upstream-127.js`（**95 断言**）：主题补丁逐条 CSS 片段 + `usageModel` 四种形状行为级 + 缓存版本 1→2 的**行为级**验证（v1 忽略 / v2 采纳）+ `inject.js` 六处定点 10 落点与 4 处旧形态消失 + `toast` 向后兼容 + Linux 三段（含上游 `open-url` 行为规范的三平台/拒非法/错误不回流 URL）。
- 评估报告：工作区 `WorkDaddy-上游1.2.7吸纳建议报告-2026-09-26.md`（含「大白话」节）+ 同名大白话 HTML。
